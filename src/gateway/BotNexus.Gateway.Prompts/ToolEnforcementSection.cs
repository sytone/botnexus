...
public static class ToolEnforcementSection
{
    ...
    [
        ...
        "For multi-step work, never claim a step's outcome before its tool result arrives: emit the tool call, wait for the real result, then report it. Waiting for a result is not a reason to end the turn — once the result is in, continue straight on to the next step.",
