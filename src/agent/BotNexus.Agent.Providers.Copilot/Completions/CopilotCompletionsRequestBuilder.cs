...
/// <summary>
/// Builds the JSON request body for the GitHub Copilot Chat Completions API. Mirrors the
/// <see cref="BotNexus.Agent.Providers.Anthropic.AnthropicRequestBuilder"/> pattern: this type owns
/// the payload-assembly logic (including the OpenRouter/Anthropic cache-control and tool-history
/// helpers, which are used only by payload assembly), while the (not-yet-extracted) message/tool
/// conversion is supplied as delegates so it can continue to live on the provider until a later
/// refactor step relocates it to a dedicated converter (#1403 step 1/6 — pure move + delegation, no
/// behavior change).
/// </summary>
internal static class CopilotCompletionsRequestBuilder
{
