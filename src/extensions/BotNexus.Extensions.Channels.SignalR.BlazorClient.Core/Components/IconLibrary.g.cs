// <auto-generated />
// Generated by scripts/generate-icons.py from assets/icons/svg. Do not edit by hand:
// change the SVG and re-run the script. See the script header for what it normalises.

namespace BotNexus.Extensions.Channels.SignalR.BlazorClient.Components;

/// <summary>The portal icon set, inlined so each icon can inherit and override colour.</summary>
public static class IconLibrary
{
    /// <param name="Stroke">Root stroke: currentColor, or url(#...) for a gradient icon.</param>
    /// <param name="Body">Inner markup, with gradient ids already made unique.</param>
    public readonly record struct IconDefinition(string Stroke, string Body);

    public static readonly IReadOnlyDictionary<string, IconDefinition> Icons =
        new Dictionary<string, IconDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ["activity"] = new("currentColor",
                """
                <path d="M3 12h3l2.1-5.2 3.7 10.4 2.5-6.2 1.7 3h5"/>
                """),
            ["add"] = new("currentColor",
                """
                <path d="M12 5v14M5 12h14"/>
                """),
            ["agents"] = new("url(#bn-agents-g)",
                """
                <defs><linearGradient id="bn-agents-g" x1="5" y1="3" x2="19" y2="21" gradientUnits="userSpaceOnUse"><stop stop-color="#3B82F6"/><stop offset="1" stop-color="#8B5CF6"/></linearGradient></defs><path d="M12 7v4M8.5 16l3.5-5 3.5 5"/><circle cx="12" cy="5" r="3"/><circle cx="7" cy="18" r="3"/><circle cx="17" cy="18" r="3"/>
                """),
            ["assistant"] = new("url(#bn-assistant-g)",
                """
                <defs><linearGradient id="bn-assistant-g" x1="4" y1="3" x2="21" y2="21" gradientUnits="userSpaceOnUse"><stop stop-color="#8B5CF6"/><stop offset="1" stop-color="#3B82F6"/></linearGradient></defs><path d="m12 2 1.6 5.4L19 9l-5.4 1.6L12 16l-1.6-5.4L5 9l5.4-1.6Z"/><path d="m19 15 .8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8Z"/>
                """),
            ["attach"] = new("currentColor",
                """
                <path d="m9 12 6.2-6.2a3 3 0 1 1 4.2 4.2l-8.5 8.5a5 5 0 0 1-7.1-7.1l8-8"/><path d="m7 14 7.5-7.5"/>
                """),
            ["avoid"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="9"/><path d="m5.6 5.6 12.8 12.8"/>
                """),
            ["back"] = new("currentColor",
                """
                <path d="M19 12H5"/><path d="m11 6-6 6 6 6"/>
                """),
            ["bot"] = new("url(#bn-bot-g)",
                """
                <defs><linearGradient id="bn-bot-g" x1="4" y1="4" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop stop-color="#06B6D4"/><stop offset="1" stop-color="#3B82F6"/></linearGradient></defs><rect x="4" y="6" width="16" height="13" rx="4"/><path d="M12 6V3M9 3h6M8 12h.01M16 12h.01M9 16h6"/>
                """),
            ["canvas"] = new("url(#bn-canvas-g)",
                """
                <defs><linearGradient id="bn-canvas-g" x1="4" y1="4" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop stop-color="#8B5CF6"/><stop offset="1" stop-color="#EC4899"/></linearGradient></defs><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M4 9h16M9 4v16M15 13l2 2-2 2"/>
                """),
            ["chat"] = new("currentColor",
                """
                <path d="M20 15a3 3 0 0 1-3 3H9l-5 3v-6a3 3 0 0 1-1-2.2V7a3 3 0 0 1 3-3h11a3 3 0 0 1 3 3Z"/>
                """),
            ["check"] = new("currentColor",
                """
                <path d="m4 12.5 5 5L20 6.5"/>
                """),
            ["close"] = new("currentColor",
                """
                <path d="M6 6l12 12M18 6 6 18"/>
                """),
            ["configuration"] = new("currentColor",
                """
                <path d="M4 6h8M16 6h4M4 12h3M11 12h9M4 18h10M18 18h2"/><circle cx="14" cy="6" r="2"/><circle cx="9" cy="12" r="2"/><circle cx="16" cy="18" r="2"/>
                """),
            ["conversation"] = new("currentColor",
                """
                <path d="M14 5H6a3 3 0 0 0-3 3v5a3 3 0 0 0 3 3h1v3l4-3h3a3 3 0 0 0 3-3V8a3 3 0 0 0-3-3Z"/><path d="M17 8h1a3 3 0 0 1 3 3v4a3 3 0 0 1-3 3h-1v2l-3-2"/>
                """),
            ["copy"] = new("currentColor",
                """
                <rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v1"/>
                """),
            ["cron-jobs"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="7"/><path d="M12 8v4l2.5 1.5M5.5 5.5H2.8v-2.7M18.5 18.5h2.7v2.7"/><path d="M4 7a9 9 0 0 1 15-2M20 17a9 9 0 0 1-15 2"/>
                """),
            ["dark-mode"] = new("currentColor",
                """
                <path d="M20 15.2A8.5 8.5 0 0 1 8.8 4a8.5 8.5 0 1 0 11.2 11.2Z"/>
                """),
            ["delete"] = new("currentColor",
                """
                <path d="M4 7h16M9 3h6l1 4M7 7l1 14h8l1-14M10 11v6M14 11v6"/>
                """),
            ["edit"] = new("currentColor",
                """
                <path d="M4 20h4L19 9a2.8 2.8 0 0 0-4-4L4 16v4Z"/><path d="m14 6 4 4"/>
                """),
            ["error"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="9"/><path d="m9 9 6 6M15 9l-6 6"/>
                """),
            ["file"] = new("currentColor",
                """
                <path d="M14 3H7a1.5 1.5 0 0 0-1.5 1.5v15A1.5 1.5 0 0 0 7 21h10a1.5 1.5 0 0 0 1.5-1.5V7.5L14 3Z"/><path d="M13.5 3v5h5"/>
                """),
            ["folder"] = new("currentColor",
                """
                <path d="M3 7.5A1.5 1.5 0 0 1 4.5 6h4.2l2 2.5h8.8A1.5 1.5 0 0 1 21 10v8a1.5 1.5 0 0 1-1.5 1.5h-15A1.5 1.5 0 0 1 3 18V7.5Z"/>
                """),
            ["guide"] = new("currentColor",
                """
                <path d="M4 5.5A3.5 3.5 0 0 1 7.5 4H11v15H7.5A3.5 3.5 0 0 0 4 20.5ZM20 5.5A3.5 3.5 0 0 0 16.5 4H13v15h3.5a3.5 3.5 0 0 1 3.5 1.5Z"/>
                """),
            ["help"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="9"/><path d="M9.3 9.4a2.8 2.8 0 1 1 3.7 2.6c-.6.2-1 .8-1 1.5v.5"/><path d="M12 17.4h.01"/>
                """),
            ["home"] = new("currentColor",
                """
                <path d="M3.5 10.5 12 3.5l8.5 7"/><path d="M5.5 9.2V20h13V9.2"/><path d="M9.5 20v-6h5v6"/>
                """),
            ["light-mode"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/>
                """),
            ["move"] = new("currentColor",
                """
                <path d="M12 2v20M2 12h20M12 2 9 5M12 2l3 3M22 12l-3-3M22 12l-3 3M12 22l-3-3M12 22l3-3M2 12l3-3M2 12l3 3"/>
                """),
            ["notifications"] = new("currentColor",
                """
                <path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0"/>
                """),
            ["pause"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="9"/><path d="M9.5 8.5v7M14.5 8.5v7"/>
                """),
            ["pin"] = new("currentColor",
                """
                <path d="m9 3 6 6-2 2 4 4-2 2-4-4-2 2-6-6 2-2 2 2 4-4-2-2ZM8 16l-5 5"/>
                """),
            ["plugins"] = new("url(#bn-plugins-g)",
                """
                <defs><linearGradient id="bn-plugins-g" x1="6" y1="3" x2="17" y2="21" gradientUnits="userSpaceOnUse"><stop stop-color="#14B8A6"/><stop offset="1" stop-color="#3B82F6"/></linearGradient></defs><path d="M8 3v5M16 3v5M6 8h12v2a6 6 0 0 1-6 6v5M9 12h6"/>
                """),
            ["refresh"] = new("currentColor",
                """
                <path d="M20 12a8 8 0 1 1-2.6-5.9"/><path d="M20 4v4.5h-4.5"/>
                """),
            ["reports"] = new("currentColor",
                """
                <path d="M6 3h8l4 4v14H6Z"/><path d="M14 3v5h4M9 17v-3M12 17v-6M15 17v-4"/>
                """),
            ["running"] = new("currentColor",
                """
                <path d="M12 3a9 9 0 1 0 9 9"/>
                """),
            ["search"] = new("currentColor",
                """
                <circle cx="10.5" cy="10.5" r="6.5"/><path d="m15.5 15.5 4.5 4.5"/>
                """),
            ["send"] = new("currentColor",
                """
                <path d="m3 11 18-8-8 18-2-8-8-2Z"/><path d="m11 13 5-5"/>
                """),
            ["skills"] = new("url(#bn-skills-g)",
                """
                <defs><linearGradient id="bn-skills-g" x1="5" y1="4" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop stop-color="#8B5CF6"/><stop offset="1" stop-color="#EC4899"/></linearGradient></defs><path d="M8.5 4.5 10 7l2.5 1.5L10 10l-1.5 2.5L7 10 4.5 8.5 7 7Z"/><path d="m15 11 1.3 3.2L19.5 15l-3.2 1.3L15 19.5l-1.3-3.2-3.2-1.3 3.2-.8Z"/><path d="m19 3 .7 1.3L21 5l-1.3.7L19 7l-.7-1.3L17 5l1.3-.7Z"/>
                """),
            ["stop"] = new("currentColor",
                """
                <circle cx="12" cy="12" r="9"/><rect x="8.5" y="8.5" width="7" height="7" rx="1"/>
                """),
            ["thinking"] = new("currentColor",
                """
                <path d="M8.2 15.5h8.3a3.75 3.75 0 0 0 .4-7.5 5.25 5.25 0 0 0-9.9-1.2 3.9 3.9 0 0 0 1.2 8.7Z"/><circle cx="6.6" cy="18.6" r="1.3"/><circle cx="10.7" cy="20.9" r="0.9"/>
                """),
            ["todo"] = new("currentColor",
                """
                <path d="m4 6 1.5 1.5L8 5M11 6h9M4 12l1.5 1.5L8 11M11 12h9M4 18l1.5 1.5L8 17M11 18h9"/>
                """),
            ["tools"] = new("currentColor",
                """
                <path d="M14.7 6.3a4 4 0 0 0-5 5L3.8 17.2a2 2 0 1 0 2.9 2.9l5.8-5.9a4 4 0 0 0 5.2-4.9l-2.6 2.6-3-3 2.6-2.6Z"/>
                """),
            ["usage"] = new("url(#bn-usage-g)",
                """
                <defs><linearGradient id="bn-usage-g" x1="4" y1="12" x2="20" y2="12" gradientUnits="userSpaceOnUse"><stop stop-color="#22C55E"/><stop offset=".55" stop-color="#F59E0B"/><stop offset="1" stop-color="#EF4444"/></linearGradient></defs><path d="M4.2 17a9 9 0 1 1 15.6 0"/><path d="m12 13 4-5M8 17h8"/>
                """),
            ["visibility"] = new("currentColor",
                """
                <path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="3"/>
                """),
            ["warning"] = new("currentColor",
                """
                <path d="M12 4 2.8 20h18.4L12 4Z"/><path d="M12 10v4M12 17.4h.01"/>
                """),
            ["workspace"] = new("currentColor",
                """
                <rect x="3" y="4" width="18" height="16" rx="3"/><path d="M3 9h18M9 9v11"/>
                """),
        };

    /// <summary>Every icon name in the set, for tests and tooling.</summary>
    public static IReadOnlyList<string> Names { get; } =
    [
        "activity",
        "add",
        "agents",
        "assistant",
        "attach",
        "avoid",
        "back",
        "bot",
        "canvas",
        "chat",
        "check",
        "close",
        "configuration",
        "conversation",
        "copy",
        "cron-jobs",
        "dark-mode",
        "delete",
        "edit",
        "error",
        "file",
        "folder",
        "guide",
        "help",
        "home",
        "light-mode",
        "move",
        "notifications",
        "pause",
        "pin",
        "plugins",
        "refresh",
        "reports",
        "running",
        "search",
        "send",
        "skills",
        "stop",
        "thinking",
        "todo",
        "tools",
        "usage",
        "visibility",
        "warning",
        "workspace",
    ];
}
