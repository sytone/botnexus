...
Assert-True ($prompt -match 'DIFFERENT PROCESSES') 'The priming prompt must state that the CLI and gateway host are distinct processes.'
Assert-True ($prompt -match 'Get-NetTCPConnection') 'The priming prompt must give port-based process discovery.'
Assert-True ($prompt -match 'UNRELIABLE') 'The priming prompt must warn that name-based process discovery is unreliable.'
...
Assert-True ($prompt -match 'SOURCE/BUILD ROOT') 'The priming prompt must identify the source/build root.'
Assert-True ($prompt -match 'CONFIG/STATE ROOT') 'The priming prompt must identify the config/state root.'
