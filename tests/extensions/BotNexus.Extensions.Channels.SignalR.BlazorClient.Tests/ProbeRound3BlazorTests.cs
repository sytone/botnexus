...
public sealed class ProbeRound3BlazorTests : IDisposable
{
    ...
    public async Task PortalLoadService_WhenRestFails_SetsLoadErrorAndIsReadyFalse()
    {
        ...
        var sut = new PortalLoadService(restClient, hub, store, eventHandler);
        await sut.InitializeAsync("http://localhost:1/hub", CancellationToken.None);
    ...
    [Fact]
    public async Task PortalLoadService_InitializeAsync_StaleCronHistory404_DoesNotBlockOtherConversationLoad()
    {
        var restClient = Substitute.For<IGatewayRestClient>();
        const string agentId = "assistant";
        const string staleCronConversationId = "cron-session:cron:20260509002033:6f2f84a4f1634ff492a4fec212872c54";
        const string healthyConversationId = "conv-healthy";
        const string staleCronSessionId = "cron:20260509002033:6f2f84a4f1634ff492a4fec212872c54";

        restClient.GetAgentsAsync(Arg.Any<CancellationToken>())
            .Returns([
                new AgentSummary(agentId, "Assistant")
            ]);
        restClient.GetConversationsAsync(agentId, Arg.Any<CancellationToken>())
            .Returns([
                new ConversationSummaryDto(
                    staleCronConversationId,
                    agentId,
                    "Stale Cron Projection",
                    false,
                    "Active",
                    null,
                    0,
        ...
        restClient.GetSessionsAsync(Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns([
                new SessionSummary(
                    staleCronSessionId,
                    agentId,
        ...
        restClient.GetSessionHistoryAsync(staleCronSessionId, 200, 0, Arg.Any<CancellationToken>())
            .Returns(_ => Task.FromException<SessionHistoryResponseDto?>(
                new HttpRequestException("Not Found", null, HttpStatusCode.NotFound)));
        restClient.GetHistoryAsync(healthyConversationId, 200, 0, Arg.Any<CancellationToken>())
            .Returns(new ConversationHistoryResponseDto(healthyConversationId, 0, 0, 200, []));
        ...
        await restClient.Received(1).GetSessionHistoryAsync(staleCronSessionId, 200, 0, Arg.Any<CancellationToken>());
        await restClient.Received(1).GetHistoryAsync(healthyConversationId, 200, 0, Arg.Any<CancellationToken>());
        var agent = store.GetAgent(agentId);
        agent.ShouldNotBeNull();
        agent!.Conversations.ContainsKey(staleCronConversationId).ShouldBeFalse();
        agent.Conversations.ContainsKey(healthyConversationId).ShouldBeTrue();
    }
