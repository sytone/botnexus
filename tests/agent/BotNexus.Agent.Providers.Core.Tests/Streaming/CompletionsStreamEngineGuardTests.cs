...
/// <summary>
/// Guards the shared Chat Completions engine (the OpenAI / Copilot Completions hot path) against
/// unbounded body reads (OOM-DoS, issue #1685). Mirrors the merged Copilot guard (#1668): the
/// untrusted SSE success stream is wrapped in <see cref="ByteCountingStream"/> (16 MiB total + 8 MiB
/// per-frame) and the non-2xx error body is read via
/// <see cref="BoundedHttpContent.ReadStringWithLimitAsync"/>. Driven through the engine with a
/// minimal transport profile so the engine's read loop is what bounds the body.
/// </summary>
public class CompletionsStreamEngineGuardTests
{
