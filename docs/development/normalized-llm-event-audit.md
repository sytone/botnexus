...
`d67942adf9e3886b7e78c9f132198408eb0272e1` on 2026-09-07. The compatible-provider safety mapping was
rechecked and exercised through its streaming fixture for #4051. This establishes agreement among the
three named OpenAI-family paths for `content_filter`; it is not a claim of universal provider parity.
...
| **Refusal / safety output** | **Reverified safety mapping:** `CompletionsStreamEngine.MapStopReason:326-341`, `ResponsesStreamPrimitives.MapStopReason:118-128`, and `OpenAICompatProvider.MapStopReason` map `content_filter` to `StopReason.Sensitive` with `"Content filtered by provider"`. The compatible fixture drives `OpenAIStreamProcessor.ParseCompatAsync` and asserts the terminal `DoneEvent` and result. | #3296 repaired the shared engine; #4051 repaired the independently supplied compatible mapper. This establishes only the three named OpenAI-family paths. Earlier refusal-text representation/loss findings belong to the historical #3295 audit and were not reverified here. | Preserve the diagnostic and compatible streaming regression. Do not infer universal provider parity or refusal-text status from this stop-reason coverage. | Shared-engine #3296 and compatible-provider #4051 resolved; #3295 status not reverified |
...
| [#3296](https://github.com/Sytone/botnexus/issues/3296) | Safety mapping | **Reverified resolved for the shared engine:** `CompletionsStreamEngine` maps `content_filter` to `Sensitive` and retains its diagnostic; Responses agrees. Compatible-provider coverage is recorded separately in #4051. |
...
|---|---|---|
| [#4051](https://github.com/Sytone/botnexus/issues/4051) | Compatible-provider safety mapping | **Resolved:** `OpenAICompatProvider.MapStopReason` maps `content_filter` to `Sensitive` with the shared diagnostic; a real compatible SSE fixture asserts the `DoneEvent` and terminal result. This does not establish behavior for unrelated provider families. |
