...
A throwing child cancellation callback does not reverse the winning `Killed` disposition. The timeout source is disposed in `finally`, and the kill owner always attempts once-only teardown independently of the caller's cancellation token. Callback exceptions are logged as warnings; `KillAsync` returns `true` for the accepted kill, not as proof that every cleanup operation succeeded. Existing stop/deletion failure diagnostics remain authoritative, and a successful deletion audit is still conditional on actual removal. Regression coverage registers the throwing callback on the real spawned-child token, races completion, checks source disposal and retirement, and rejects duplicate teardown.
