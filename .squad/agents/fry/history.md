...
### 2026-07-31 — Tool Output ANSI Stripping + JSON Unicode Fix
Issue #289: Tool results from agents contain raw ANSI terminal escape codes (color, cursor movement, etc.) that render as garbage in the `<pre>` tag. Created `AnsiStripper.Strip()` (source-generated regex) in `Services/AnsiStripper.cs` to strip CSI, OSC, and single-char escapes. Applied at all ToolResult assignment sites: `GatewayEventHandler`, `PortalLoadService`, and `AgentInteractionService`. Also fixed JSON tool args rendering by adding `JavaScriptEncoder.UnsafeRelaxedJsonEscaping` to the shared `s_jsonOptions` so `&` renders as `&` not `\u0026`. 8 unit tests for AnsiStripper.

### 2026-05-15 — Copy-to-Clipboard for Tool Output Sections
Issue #289 follow-up: Added copy buttons (📋) to expanded tool call Arguments and Result sections in ChatPanel.razor. Reused existing `BotNexus.copyToClipboard` JS interop and the same feedback pattern (✓ for 2 seconds) as the message-level copy button. Used separate `_copiedSectionId` field with `{msgId}-args`/`{msgId}-result` composite keys so tool-section and message-level copy states don't collide. CSS uses `.tool-section:hover` to reveal the button (same opacity approach as `.msg-copy-btn`). 5 new bUnit tests covering render, click, and per-section clipboard invocations.
